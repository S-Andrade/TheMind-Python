...
def whoNext(cards0, cards1,cards2):

    if len(cards0) == 0: cards0 = [1001]
    if len(cards1) == 0: cards1 = [1001]
    if len(cards2) == 0: cards2 = [1001]

    first_elements = [cards0[0], cards1[0], cards2[0]]
    m = min(first_elements)
    
    if m  == 1001:
        return "nocards"

    else:
        lowest_index = first_elements.index(m)
        return "player"+str(lowest_index)
    ...
    while True:
        ...
        if playcard != "":
            ...
            if  targetPlayer == "":
                ...
                next = whoNext(cards0, cards1, cards)
                print(next)

                x = random.randint(0,100)
                print(x)

                if next == targetPlayer and x < 70:
                    time.sleep(0.7)

                    message = f'SetPosture,player2,hope'
                    client_socket.sendall(message.encode('utf-8'))

                    time.sleep(0.3)

                    message = f'GazeAtTarget,U{targetPlayer}'
                    client_socket.sendall(message.encode('utf-8'))

                    time.sleep(0.3)

                    message = f'GazeAtTarget,D{targetPlayer}'
                    client_socket.sendall(message.encode('utf-8'))
                    ...
                    message = f'SetPosture,player2,neutral'
                    client_socket.sendall(message.encode('utf-8')) 
            ...
            if  targetPlayer != "":
                ...
                if time.time() - timeGaze >= nextTimeToLook + gazeTime:
                    ...
                    if next != "player2" and next != "nocards" and x < 70:
                        time.sleep(0.7)

                        message = f'SetPosture,player2,fear'
                        client_socket.sendall(message.encode('utf-8'))

                        time.sleep(0.3)

                        message = f'GazeAtTarget,Rmainscreen'
                        client_socket.sendall(message.encode('utf-8'))

                        time.sleep(0.3)

                        message = f'GazeAtTarget,Lmainscreen'
                        client_socket.sendall(message.encode('utf-8'))
